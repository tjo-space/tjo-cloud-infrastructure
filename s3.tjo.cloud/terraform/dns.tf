resource "desec_rrset" "api" {
  for_each = {
    A    = [for k, v in local.nodes_deployed : v.public_ipv4 if v.garage_kind == "gateway" && v.use == true]
    AAAA = [for k, v in local.nodes_deployed : v.public_ipv6 if v.garage_kind == "gateway" && v.use == true]
  }
  domain  = "tjo.cloud"
  subname = "api.s3"
  type    = each.key
